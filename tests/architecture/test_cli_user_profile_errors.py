...
@pytest.mark.parametrize(
    "invalid_value",
    ["two", None, True, 0, -1, 1.5, float("inf")],
)
def test_invalid_window_size_exits_cleanly(
    tmp_path,
    monkeypatch,
    capsys,
    invalid_value,
    ...
    profile_path = tmp_path / "user_profile.json"
    profile_path.write_text(
        json.dumps({"preferences": {"window_size": invalid_value}}),
        encoding="utf-8",
    )
    original_content = profile_path.read_bytes()
    monkeypatch.setenv("ASTROPILOT_DATA_DIR", str(tmp_path))
    ...
    assert exit_info.value.code == 2
    assert captured.out == ""
    assert "preferences.window_size" in captured.err
    assert "entier strictement positif" in captured.err
    assert "Traceback" not in captured.err
    assert profile_path.read_bytes() == original_content
...
@pytest.mark.parametrize(
    "invalid_value",
    ["", "unknown", "science", "finisher", None, True, []],
)
def test_invalid_decision_mode_exits_cleanly(
    tmp_path,
    monkeypatch,
    capsys,
    invalid_value,
    ...
    profile_path = tmp_path / "user_profile.json"
    profile_path.write_text(
        json.dumps({"preferences": {"decision_mode": invalid_value}}),
        encoding="utf-8",
    )
    original_content = profile_path.read_bytes()
    monkeypatch.setenv("ASTROPILOT_DATA_DIR", str(tmp_path))
    ...
    assert exit_info.value.code == 2
    assert captured.out == ""
    assert "preferences.decision_mode" in captured.err
    assert "mode reconnu attendu" in captured.err
    assert "Traceback" not in captured.err
    assert profile_path.read_bytes() == original_content
