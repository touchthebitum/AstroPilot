...
def row(valid_at, *, temperature, humidity, wind, **extra):
    return {
        "time": valid_at,
...
def test_preserves_snapshot_provenance_without_inventing_model_or_site_altitude():
    ...
    evidence = build_decision_forecast_evidence(
        source,
        [row(valid_at, temperature=6.0, humidity=85.0, wind=9.0)],
    )
...
def test_excludes_past_row_and_includes_row_exactly_at_retrieval_time():
    rows = [
        ...
        row(RETRIEVED, temperature=5.0, humidity=89.0, wind=6.0),
    ]

    evidence = build_decision_forecast_evidence(snapshot(), rows)

    assert len(evidence.forecast_points) == 3
    assert all(
...
def test_emits_only_temperature_humidity_and_mean_wind():
    evidence = build_decision_forecast_evidence(
        snapshot(),
        [
            row(
